...
docker run \
    ...
    /bin/bash -c "cd /app && nosetests --with-coverage --cover-branches --cover-package=$("${SCRIPT_DIR_NAME}/repo.sh" -u) && if [ -e /app/.coverage ]; then sed -i -e 's|/app/|$REPO_ROOT_DIR/|g' /app/.coverage; fi"

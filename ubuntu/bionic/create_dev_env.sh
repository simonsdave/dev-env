...
#
#   #!/usr/bin/env bash
#   curl -s https://raw.githubusercontent.com/simonsdave/dev-env/release-0.5.14/ubuntu/bionic/create_dev_env.sh | bash -s -- "$@"
#   exit $?
#
# to test
#
...
if [ $# != 4 ]; then
    echo "usage: $(basename "$0") [--memory <MB>|--timezone <timezone>] <github username> <github email> <github public key> <github private key>" >&2
    exit 1
fi
...
curl -s --output Vagrantfile "https://raw.githubusercontent.com/simonsdave/dev-env/release-0.5.14/ubuntu/bionic/Vagrantfile"
